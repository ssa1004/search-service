 ...
 *
 * <p>match finder — 운영 ES 모드일 때만 등록. 메모리 모드는 별도 시뮬레이션이 필요하면 추가.</p>
 *
 * <p>alert publisher (Kafka) — {@code search.kafka.enabled=true} 일 때만 등록. 그 외 환경 (local
 * dev) 은 publisher 빈이 없어 SavedSearchEvaluatorJob 도 의존성 미해소로 비활성 — 명시적이라 안전.</p>
 */
@Configuration
public class SavedSearchConfig {

    @Bean
    public SearchQueryJsonCodec searchQueryJsonCodec(ObjectMapper objectMapper) {
        return new SearchQueryJsonCodec(objectMapper);
    }

    @Bean
    @ConditionalOnProperty(name = "search.engine", havingValue = "elasticsearch", matchIfMissing = true)
    public SavedSearchMatchFinder savedSearchMatchFinder(ElasticsearchClient client,
                                                         SearchIndexProperties properties) {
        return new ElasticsearchSavedSearchMatchFinder(client, properties);
    }

    @Bean
    @ConditionalOnProperty(name = "search.kafka.enabled", havingValue = "true")
    public SavedSearchAlertPublisher savedSearchAlertPublisher(
            KafkaTemplate<String, String> kafkaTemplate, ObjectMapper objectMapper) {
        return new KafkaSavedSearchAlertPublisher(kafkaTemplate, objectMapper);
    }
    ...
    public EvaluateSavedSearchesUseCase evaluateSavedSearches(
            SavedSearchRepository repository,
            SavedSearchMatchFinder matchFinder,
            SavedSearchAlertPublisher publisher,
            Clock clock) {
        return new EvaluateSavedSearchesService(repository, matchFinder, publisher, clock);
    }

    @Bean
    @ConditionalOnBean(EvaluateSavedSearchesUseCase.class)
    public SavedSearchEvaluatorJob savedSearchEvaluatorJob(EvaluateSavedSearchesUseCase useCase,
                                                           MeterRegistry meterRegistry) {
        return new SavedSearchEvaluatorJob(useCase, meterRegistry);
    }
}
