...
검색 클릭 로그 기반 boost (function_score)(= 글자 매칭 점수에 인기·신상 같은 추가 점수를 곱해 더 좋은 상품을 위로 올리는 방식), CDC 인덱싱 파이프라인(= source DB의 변경을 실시간으로 잡아채 검색엔진에 자동 반영하는 흐름), 운영 중 무중단 reindex
(alias swap)(= 검색은 'products' 별명으로만 부르고, 새 인덱스를 다 채운 뒤 별명이 가리키는 대상만 한순간에 바꿔 검색이 한순간도 안 멈추게 하는 방식) 를 제공합니다.
...
version 비교(= 상품마다 버전 번호를 붙여 더 낮거나 같은 버전이 들어오면 ES가 거부해, 같은 메시지가 두 번 색인돼도 재고가 두 번 늘지 않게 하는 것)로 같은 메시지가 두 번 처리되어도 결과 정합. (ADR-0004)
...
- **인기도** — `log1p(clickCount) * popularityWeight`(= 클릭 수에 로그를 씌워 1000번과 10000번 차이가 10배가 아니라 살짝만 나게 해, 한 상품이 점수를 독식하지 않게 함) (log 함수라 클릭 수 폭증해도 안정).
- **신상품** — 출시일 origin 의 `gauss decay`(= 출시일이 오래될수록 점수를 서서히 깎아, 신상품을 위에 올리되 어느 날 갑자기 뚝 떨어지지 않고 자연스럽게 밀려나게 함) (반감기 30일 default).
...
nori 한국어 형태소(= 한국어 문장을 '의미 단위'로 쪼개 '나이키의'에서 조사 '의'를 떼어 '나이키'와 매칭하게 하는 한국어 전용 분석기), user_dictionary(= '덩크로우' 같은 브랜드명은 쪼개지 말고 한 덩어리로 지키라고 등록하는 사용자 사전).
...
운영자가 등록한 동의어 그룹을 ES `synonym_graph`(= '조던1 = 에어조던1 = Air Jordan 1'처럼 같은 뜻 단어를 한 묶음으로 등록해 어느 표기로 검색해도 다 찾게 하는 필터) filter 로 빌드, alias swap 으로 무중단
...
> ```bash
> make run                       # 앱 메모리 모드 부팅 (ES/Kafka 없이 — :8080)
> make demo                      # 다른 셸에서 — 검색/자동완성/동의어/reindex/분석 한 cycle
> # 운영 모드: make up (인프라) → 다른 셸에서 make run-es
> ```
...
> Swagger UI / `demo.sh` 출력 캡처를 재현 가능하게 찍는 절차는
> [`docs/screenshots/README.md`](docs/screenshots/README.md) 참고 (메모리 모드라 Docker 불필요).
...
> **Kafka listener 설계**: 호스트에서 `./gradlew bootRun` 으로 띄운 앱은 `localhost:9092` 로,
...
> 무한 대기/실패합니다 — CDC outbox relay 가 Kafka 로 발행하지 못하는 흔한 함정.
...
> 단위/슬라이스 테스트(domain·application·adapter·bootstrap)는 JUnit 5 + Java 로 작성되어
> 있고, e2e/IT 테스트만 Kotlin 입니다. 운영 코드는 전부 Kotlin 이며 테스트 마이그레이션은
> 진행 중입니다 — 위 표의 숫자는 테스트 케이스 수입니다.
...
| 시나리오 | endpoint | 측정 |
|---|---|---|
| `search-simple` | `POST /api/v1/search/products` | 단순 검색 latency, 500 req/s |
| `search-with-filters` | `POST /api/v1/search/products` (filter+facet+sort) | query DSL 비용, 200 req/s |
| `saved-search-create` | `POST /api/v1/saved-searches` (REST 미노출 — SaveSearchService 서비스만 존재, 컨트롤러 미와이어링) | write throughput, 0 → 50 VU |
| `pagination` | `POST /api/v1/search/products` (`page`/`size`, from+size 방식) | 깊은 페이지 latency |
...
## 운영 모드 (`SEARCH_ENGINE=elasticsearch`)
...
Spring Boot Actuator + Micrometer 로 metric / health / trace 를 노출합니다. Prometheus 가
`/actuator/prometheus` 를 scrape 하고, Kubernetes kubelet 은 `/actuator/health/{liveness,readiness}`
를 probe 로 사용합니다 (ADR-0010). CDC 컨슈머는 `cdc.consume` / `cdc.dlt` Micrometer counter 를
발행합니다.

![Observability topology](docs/diagrams/observability.svg)

> 다이어그램 소스는 [`docs/diagrams/observability.mmd`](docs/diagrams/observability.mmd) (Mermaid).
> 재렌더: `npx @mermaid-js/mermaid-cli -i docs/diagrams/observability.mmd -o docs/diagrams/observability.svg -b transparent`
