...
- **서킷 브레이커(Circuit Breaker)** — 외부 호출이 자꾸 실패하면 두꺼비집처럼 잠시 회선을 끊어 즉시 실패시키고, 죽은 서버를 계속 두드려 같이 쓰러지는 걸 막는 장치.
- **Retry(재시도)** — 일시적 실패(잠깐의 끊김)는 몇 번 다시 시도해 자동으로 넘기는 것. 단, 회복 불가능한 실패엔 의미 없으니 횟수를 제한한다.
- **decorator(데코레이터) / wrap** — 원래 호출을 그대로 두고 그 바깥에 재시도·차단 같은 기능을 한 겹 덧씌우는 방식. 선물 위에 포장지를 두르는 것.
- **half-open(반열림)** — 끊었던 회로를 완전히 다시 열기 전, 시험 삼아 몇 건만 흘려보내 회복됐는지 떠보는 중간 상태.
- **jitter / thundering herd** — 여러 요청이 똑같은 시점에 한꺼번에 재시도해 서버를 다시 덮치는 게 thundering herd(우르르 몰림). 재시도 시점을 일부러 들쭉날쭉(jitter) 흩뜨려 막는다.
- **bulkhead(격벽)** — 배의 칸막이처럼, 느린 ES 호출이 스레드를 다 잡아먹어도 다른 기능까지 못 쓰게 되는 걸 막는 동시 호출 수 제한.
- **p99 (99 백분위)** — 응답 시간을 줄 세웠을 때 위에서 1%만 더 느린 지점. "가장 느린 축에 드는 사용자 체감"을 보는 지표.
