...
- **fork(포크)** — 남이 만든 오픈소스를 그대로 복제해 별도 방향으로 따로 키워가는 것. OpenSearch는 AWS가 Elasticsearch를 복제해 갈라져 나온 버전이다.
- **SSPL / Elastic License 2.0** — Elastic이 새로 건 사용 조건. 쉽게 말해 "ES 자체를 클라우드로 남에게 빌려주고 돈 받는 재판매"만 막고, 우리처럼 검색 결과만 쓰는 건 자유다.
- **function_score** — 검색 결과 순서를 매길 때 글자 매칭 점수에 '추가 점수'를 곱해 인기·신상 상품을 위로 올리는 ES 기능.
- **ILM (Index Lifecycle Management)** — 인덱스가 오래되면 '뜨거운(자주 쓰는)→식은(가끔)→차가운(거의 안 봄)' 단계로 자동 이동·정리하는 수명 관리 정책.
- **DSL / typed builder** — 검색 질의를 글자 문자열이 아니라 코드 객체로 또박또박 조립하는 방식. 오타가 컴파일 단계에서 잡혀 안전하다.
