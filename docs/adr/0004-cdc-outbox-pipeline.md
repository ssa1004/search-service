...
- **CDC (Change Data Capture)** — DB에 생긴 변경을 5분마다 직접 훑지 않고, DB의 변경 기록을 실시간으로 흘려보내 잡아채는 기법. 우편함을 직접 확인 vs 우편물 오는 즉시 알림.
- **Debezium(데비지움)** — CDC를 해 주는 대표 오픈소스 도구. 강력하지만 별도 클러스터를 운영해야 해서 작은 규모엔 부담이 크다.
- **Outbox 패턴** — DB 저장과 메시지 발송이 따로 놀아 한쪽만 성공하는 사고를 막으려, 보낼 메시지를 같은 트랜잭션 안에서 '편지함' 테이블에 같이 적고 나중에 일꾼이 꺼내 보내는 방법.
- **relay / polling(릴레이·폴링)** — 편지함 테이블을 짧은 주기(5초)로 들여다보며 '아직 안 보낸 편지'를 꺼내 Kafka로 보내고 '보냄' 도장을 찍는 백그라운드 일꾼.
- **atomicity(원자성)** — 여러 작업을 '전부 되거나 전부 안 되거나'로 묶어 절반만 성공하는 어긋남이 없게 하는 것.
- **at-least-once / 멱등(idempotent)** — 메시지가 최소 한 번은 가지만 가끔 두 번 갈 수 있는 게 at-least-once. 그래도 결과가 안 깨지게 "두 번 와도 한 번 한 것과 똑같이" 처리하는 게 멱등.
- **external version(외부 버전)** — 상품마다 버전 번호를 붙여, 더 낮거나 같은 번호가 또 오면 ES가 무시하게 하는 장치. 같은 색인이 두 번 와도 안전.
