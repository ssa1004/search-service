...
- **retention(보존 정리)** — 다 처리된 옛날 데이터를 일정 기간(여기선 7일)만 두고 그 이후엔 지워 테이블이 무한정 불어나지 않게 하는 정책.
- **ShedLock / 멀티 인스턴스 보호** — 같은 정리 작업을 여러 서버가 동시에 돌리지 않게, DB에 '내가 지금 한다'는 잠금 표를 걸어 딱 한 대만 실행하게 하는 도구.
- **cron(크론)** — '매일 새벽 3시'처럼 정해진 시각에 작업을 자동 실행하는 스케줄 설정.
- **vacuum(배큠)** — Postgres가 지운 데이터의 빈자리를 청소해 정리하는 작업. 지울 게 많으면 이 비용이 커진다.
- **WAL (Write-Ahead Log)** — DB가 변경을 본 데이터에 반영하기 전 먼저 적어 두는 기록 장부. 쌓일수록 보관 부담이 는다.
- **clock skew(시계 오차)** — 서버마다 시계가 조금씩 다른 현상. 잠금을 'DB 시계' 기준으로 잡아 이 오차에 휘둘리지 않게 한다.
