...
- 모듈 경계가 빌드 단계에서 강제되므로 `domain → adapter` 같은 역방향 import 가 컴파일
  에러로 차단된다.
- 모듈 분리 비용이 있다 — DTO / 매퍼가 layer 마다 늘어나고, 처음 보는 사람이 흐름을 잡기
  전에 모듈 경계를 한 번 학습해야 한다.

## 용어 풀이 (쉽게)

- **헥사고날 아키텍처(port/adapter)** — 핵심 로직을 한가운데 두고 DB·Kafka·웹은 콘센트(port)와 플러그(adapter)로만 연결해, 바깥을 바꿔도 안쪽 코드는 안 건드리는 구조.
- **port / adapter** — port 는 "여기 이런 기능이 꽂힌다"는 콘센트(인터페이스), adapter 는 거기에 끼우는 실제 플러그(ES·DB 같은 구현체). 콘센트 규격만 맞으면 플러그를 갈아끼울 수 있다.
- **유스케이스(use case)** — "상품을 검색한다", "변경을 색인한다" 같은 앱이 하는 한 가지 일의 단위. 화면이나 DB와 무관한 '할 일' 자체.
- **Testcontainers** — 테스트할 때 진짜 DB·ES를 도커 컨테이너로 잠깐 띄웠다가 끝나면 지우는 도구. 가짜 대신 실제와 똑같은 환경에서 검증한다.
