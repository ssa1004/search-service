...
- **facet(패싯)** — 검색 결과 옆에 '브랜드별·가격대별 개수'를 묶어 보여 주는 필터 메뉴. 쇼핑몰 왼쪽의 '브랜드(나이키 12, 아디다스 8)' 같은 목록.
- **aggregation(집계)** — 결과를 그룹지어 세거나 통계 내는 ES 기능. facet을 만드는 재료.
- **cardinality(카디널리티)** — 어떤 값의 '서로 다른 종류 수'. 색깔은 종류가 적지만(낮음), 상품 ID는 다 달라서(높음) 그룹지으면 메모리를 잡아먹는다.
- **size 상한** — 그룹 개수를 몇 개까지 받을지 한도. size:1000000 같은 위험한 값을 막아 ES 메모리가 안 터지게 한다.
- **OOM (Out Of Memory)** — 메모리가 다 차서 프로세스가 뻗는 사고.
- **HyperLogLog** — '대략 몇 종류더라'를 적은 메모리로 빠르게 어림셈하는 기법. 정확하진 않지만 수백만 종류도 가볍게 센다.
