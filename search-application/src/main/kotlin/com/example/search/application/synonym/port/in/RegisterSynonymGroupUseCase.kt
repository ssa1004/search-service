 ...
 *
 * 등록 즉시 ES 에 반영하지 않는 이유: 운영자가 여러 그룹을 한꺼번에 추가 / 삭제 후 한 번만
 * reload 하는 게 ES 인덱스 close → open 비용 측면에서 유리. [ApplySynonymsToIndexUseCase]
 * 가 별도로 호출된다.
 */
interface RegisterSynonymGroupUseCase {

    fun register(command: RegisterSynonymGroupCommand): SynonymGroup
}
