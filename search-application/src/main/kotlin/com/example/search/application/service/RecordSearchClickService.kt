 ...
 *
 * 두 단계:
 * 1. [SearchClickRepository.save] — 상세 로그 저장 (트랜잭션 안).
 * 2. [IndexWriterPort.incrementClickCount] — ES 문서의 clickCount += 1 (partial update,
 *    트랜잭션 밖 — ES 는 트랜잭션이 없음).
 *
 * ES 호출 실패가 트랜잭션을 롤백시키지 않도록 두 단계가 의도적으로 분리된다 — DB 는 commit
 * 되어 있고, ES 갱신은 retry 로 따라잡는다 (eventually consistent). ES 에 click count 가 좀 늦게
 * 반영되는 것이 클릭 기록 자체가 사라지는 것보다 낫다.
 */
@Service
class RecordSearchClickService(
    private val clicks: SearchClickRepository,
    private val indexWriter: IndexWriterPort,
    private val clock: Clock
) : RecordSearchClickUseCase {

    @Transactional
    override fun record(command: RecordSearchClickCommand) {
        val click = SearchClick(
            command.searchId,
            command.userId,
            command.productId,
            command.keyword,
        ...
        clicks.save(click)
        log.debug(
            "click recorded user={} product={} keyword='{}' rank={}",
    ...
    companion object {
        private val log = LoggerFactory.getLogger(RecordSearchClickService::class.java)
    }
}
