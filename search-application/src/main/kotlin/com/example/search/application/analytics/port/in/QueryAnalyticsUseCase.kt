 ...
 *
 * 한 인터페이스로 묶은 이유 — 같은 데이터 소스 (search_events + search_clicks) 에서 같은 구간
 * 입력으로 4가지를 보는 운영 화면 한 곳이 주된 호출자다. 분리하면 컨트롤러가 4개 빈 주입 받는
 * 보일러플레이트만 늘어남.
 */
interface QueryAnalyticsUseCase {
    ...
    fun queryLatencyPercentiles(from: Instant, to: Instant): LatencyPercentiles

    fun clickThroughRate(from: Instant, to: Instant): ClickThroughRate
}
