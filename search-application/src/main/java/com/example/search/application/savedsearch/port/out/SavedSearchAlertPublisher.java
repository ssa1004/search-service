 ...
 *
 * <p>실패 시 throw — 호출자 (스케줄러) 가 lastEvaluatedAt 갱신을 건너뛰고 다음 사이클에 재시도.
 * 다만 webhook 의 4xx 영구 실패는 무한 재시도 방지 위해 구현체가 dead-letter 처리.</p>
 */
public interface SavedSearchAlertPublisher {
    void publish(SavedSearchAlert alert, NotifyChannel channel);
}
