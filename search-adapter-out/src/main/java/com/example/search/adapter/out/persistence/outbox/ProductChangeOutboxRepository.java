...
public interface ProductChangeOutboxRepository extends JpaRepository<ProductChangeOutboxEntity, Long> {
     ...
     *
     * <p>Postgres / H2 둘 다 지원하는 표준 형태. JPA 가 생성하는 native delete 를 한정된 row 수
     * 안에 끝내기 위해 PK 부분 query 후 in() 으로 위임.</p>
     */
    @Modifying
    @Transactional
    @Query(value = "DELETE FROM product_change_outbox WHERE id IN ("
