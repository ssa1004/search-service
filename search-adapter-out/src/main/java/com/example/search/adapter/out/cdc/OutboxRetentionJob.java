 ...
 * (ADR-0014).
 *
 * <p>매일 03:00 KST 에 실행. 멀티 인스턴스 환경에서 ShedLock 으로 한 인스턴스만 실행 보장.</p>
 *
 * <p>batch 1000 row 단위로 반복 — 한 transaction 이 너무 커지지 않게 (long-running transaction
 * → leak detection 경고 / WAL 부담).</p>
 */
@Component
...
public class OutboxRetentionJob {
    ...
    public void purgeOldRows() {
        ...
        meterRegistry.counter("outbox.retention.deleted").increment(totalDeleted);
        log.info("outbox retention 완료 cutoff={} deleted={}", cutoff, totalDeleted);
    }

    @Transactional
    int deleteBatch(Instant cutoff) {
        return outbox.deletePublishedBefore(cutoff, BATCH_SIZE);
    }
}
